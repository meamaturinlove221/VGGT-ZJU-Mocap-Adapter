...
def sort_numeric(values) -> list[str]:
    ...
    return sorted([str(value) for value in values], key=key_fn)


def camera_group_name(camera_id: str | int) -> str:
    cam_num = int(camera_id)
    return "Camera_5mp" if cam_num < 48 else "Camera_12mp"
...
def build_context(dataset_path: Path, subset_name: str = SUBSET_NAME) -> DatasetContext:
    dataset_path = dataset_path.resolve()
    dataset_dir = dataset_path.parent if dataset_path.name == subset_name else dataset_path
    subset_roots = find_subset_roots(
        dataset_path if dataset_path.name == subset_name else dataset_dir,
        subset_name,
    )
    ...
    return DatasetContext(
        ...
        outer_zips=outer_zips,
        zip_index=build_zip_index(outer_zips),
    )
...
def materialize_rgb_cams_smc(
    context: DatasetContext,
    sequence_id: str,
    temp_dir: Path,
) -> tuple[Path | None, str]:
    extracted = locate_extracted_file(
        context,
        f"{context.subset_name}/rgb_cams/{sequence_id}_rgb_cams.smc",
    )
    if extracted is not None:
...
def require_h5py():
    try:
        ...
        raise SystemExit("h5py is required for this command.") from exc
    return h5py
...
def load_camera_summary(smc_path: Path) -> dict[str, object]:
    ...
    with h5py.File(smc_path, "r") as handle:
        ...
        camera_ids = [
            ...
            for camera_id in sort_numeric(handle["Camera_Parameter"].keys())
        ]
...
def resolve_required_file(context: DatasetContext, canonical_path_str: str, temp_dir: Path) -> Path:
    status = describe_expected_file(context, canonical_path_str)
    if status["status"] == "extracted":
        return Path(status["path"])
    if status["status"] == "archived":
        materialized = materialize_archived_member(context, canonical_path_str, temp_dir)
        if materialized is not None:
            return materialized
    raise FileNotFoundError(f"Could not resolve required file: {canonical_path_str}")


def load_frame_ids(main_smc: Path, camera_id: str | int) -> list[str]:
    h5py = require_h5py()
    cam_id = normalize_camera_id(camera_id)
    group_name = camera_group_name(cam_id)
    with h5py.File(main_smc, "r") as handle:
        return sort_numeric(handle[group_name][str(int(cam_id))]["color"].keys())
...
def command_inventory(args: argparse.Namespace) -> int:
    ...
    categories: dict[str, dict[str, int]] = defaultdict(
        lambda: {"expected": 0, "local": 0, "missing": 0}
    )
    missing_files = []
    local_count = 0
    ...
    for category, counts in sorted(report["categories"].items()):
        print(
            f"  - {category}: expected={counts['expected']} "
            f"local={counts['local']} missing={counts['missing']}"
        )
    if args.json_out:
        ...
        with zipfile.ZipFile(zip_path) as archive:
            for info in archive.infolist():
                ...
                if (
                    ...
                    and output_path.stat().st_size == info.file_size
                ):
                    skipped += 1
                    continue
            ...
            with zipfile.ZipFile(inner_zip) as archive:
                for info in archive.infolist():
                    if info.is_dir():
                        continue
                    output_path = (
                        target_root / args.subset_name / "rgb_cams" / Path(info.filename).name
                    )
                    if (
                        args.skip_existing
                        and output_path.exists()
...
def command_manifest(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        target_camera = (
            normalize_camera_id(args.target_camera)
            if args.target_camera
            else (available_cameras[0] if available_cameras else None)
        )
        source_cameras = (
            [normalize_camera_id(camera) for camera in args.source_cameras]
            if args.source_cameras
            else []
        )
        ...
        manifest = {
            ...
            "status": {
                "ready_for_rgb_bridge": files["main_smc"]["status"] != "missing"
                and camera_summary is not None,
                "ready_for_mask_bridge": files["annotations_smc"]["status"] != "missing"
                and camera_summary is not None,
        ...
        if blocking and not args.allow_partial:
            raise SystemExit(
                "Manifest generated, but the case is still blocked. "
                "Re-run with --allow-partial to keep this as a successful preflight."
            )
        return 0
    finally:
        temp_handle.cleanup()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inventory and bridge helper for the DNA-Rendering 4K4D subset."
    )
    parser.add_argument(
        "--subset-name",
        default=SUBSET_NAME,
        help="Subset name. Default: data_used_in_4K4D",
    )
    ...
    inventory.add_argument(
        "--dataset-path",
        required=True,
        help="Folder containing the raw dataset zips or an extracted subset root.",
    )
    ...
    assemble.add_argument(
        "--dataset-path",
        required=True,
        help="Folder containing the raw dataset zip parts.",
    )
    assemble.add_argument(
        "--target-root",
        required=True,
        help="Destination parent folder for extracted files.",
    )
    assemble.add_argument(
        "--skip-existing",
        action="store_true",
        help="Skip files that already exist with the same size.",
    )
    assemble.add_argument(
        "--extract-inner-rgb-cams",
        action="store_true",
        help="Also extract the embedded rgb_cams zip into rgb_cams/*.smc.",
    )
    ...
    manifest.add_argument("--frame", default="0", help="Frame id to record in the manifest.")
    manifest.add_argument("--target-camera", help="Target camera id. Example: 00")
    manifest.add_argument(
        "--source-cameras",
        nargs="*",
        default=[],
        help="Explicit source camera ids.",
    )
    manifest.add_argument(
        "--auto-sources",
        type=int,
        default=6,
        help="Auto-pick N source cameras if --source-cameras is omitted.",
    )
    manifest.add_argument(
        "--materialize-archived",
        action="store_true",
        help="Temporarily extract archived main/annotation SMCs for one-case probing.",
    )
    manifest.add_argument(
        "--allow-partial",
        action="store_true",
        help="Exit successfully even if the case is still blocked.",
    )
